...
def LVPassiveFilling(geoparams, hoparams, sampleid):
	"""
	sampleid: sample id
	...
	f_0, s_0, n_0 = geoparams['f0'], geoparams['s0'], geoparams['n0']
	...
	#np.savetxt(ffdf, f_d_matrix - 1, fmt = '%.10f')	
	...
	print("nodes to compute averaged fiber-stretch: ", p_rand)

	print(np.shape(outdef_rand))
	print(outdef_rang)
	print(outdef_avg)
	...
	#result_vol = fitmodel(2, 1, vn, outpress)
	#if(outdef_avg[0] < 0):
	#	outdef_avg[0] = outdef_avg[0] * (-1)
	#result_def = fitmodel(2, 1, outdef_avg, outpress)
...
if __name__ == "__main__":
	...
	npar = 3        # number of input parameter  
	...
	os.mkdir("output/")
	f_data = open(fd,'w')
	f_data.write('# q1, q2, q3, alfa1, beta1, alfa2, beta2, vol, fibstretch \n')
	...
	for i in range(len(samples.T)):
		...
		Z = samples.T[i,:]
		q1,q2,q3 = float(Z[0]), float(Z[1]), float(Z[2])
		...
		qin = np.array([q1,q2,q3])

		# solve model
		print("Rodando caso %d: (%f,%f,%f,%f)" % (i,a, b, af, bf))
		out = LVPassiveFilling(mesh, base, endo, epi, ds, nmesh, f0,s0, n0, i, a, b, af, bf)

		# senao deu erro, salva os dados
		if out is not None:
			evals.append(out)
			np.savetxt(f_data, np.concatenate((qin,out)).reshape((1,9)), fmt='%16.8e', delimiter=',')
		else:
