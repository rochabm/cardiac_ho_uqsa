...
def pce_prediction(surrogate, samples, outputs, out_index, out_label):
	...
	for i in range(n):
		...
		qoi_true[i] = respostas_test[i,out_index]
...
if __name__ == "__main__":
	...
	scatter_inputs_outputs(samples, outputs)	
	...
	sobol_m = np.zeros((4,6))
	sobol_t = np.zeros((4,6))
	...
	np.savetxt(outdir_train + "sobol_main.txt", sobol_m)
	np.savetxt(outdir_train + "sobol_total.txt", sobol_t)
