...
def CreateMesh():
	...
	boundaries = MakeSubDomains(mesh)
...
def ComputeVolume(u,J,F,mesh,n_mesh,ds):
...
def LVPassiveFilling(geoparams, hoparams, sampleid=0):
	"""
	Solve the LV passive filling problem with the HO model
	"""
	...
	print("nodes for averaged fiber-stretch:", p_rand)
	
	#normalização
	vn = np.zeros(len(outvol))
	vn[:] = (outvol[:] - outvol[0])/outvol[0]

	#print("deformacao", outdef_avg)
	#respostas
	#result_vol = fitmodel(2, 1, vn, outpress)
	#print("vn", vn)
	#print("result vol", result_vol)
	#if(outdef_avg[0] < 0):
...
if __name__ == "__main__":
    ...
    out = LVPassiveFilling(geoparams, hoparams)
